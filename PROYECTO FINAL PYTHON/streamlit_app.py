...
# El parámetro de ancho cambió de `use_container_width` a `width="stretch"`.
# Se resuelve leyendo la firma real, de modo que la app no lance TypeError
# ni avisos de obsolescencia en ninguna versión.
    ...
    if not TARJETA:
        ...
        if figuras:
            ...
            for i, figura in enumerate(figuras):
                with columnas[i % 2]:
                    st.image(str(figura), caption=figura.stem.replace("_", " ").title())
        else:
            st.info("Ejecute el cuaderno para generar las figuras en `reports/figuras/`.")
